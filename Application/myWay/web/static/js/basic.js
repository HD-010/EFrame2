 ...
 * M['metinfo_version'] ç³»ç»Ÿå½“å‰ç‰ˆæœ¬
 * M['user_name']   å½“å‰é¡µé¢ç™»å½•ç”¨æˆ·å
 * M['device_type'] å®¢æˆ·ç«¯åˆ¤æ–­ï¼ˆdï¼šPCç«¯ï¼Œtï¼šå¹³æ¿ç«¯ï¼Œmï¼šæ‰‹æœºç«¯ï¼‰
 * met_prevarrow,
   met_nextarrow    slickæ’ä»¶ç¿»é¡µæŒ‰é’®è‡ªå®šä¹‰html
 */
// ç½‘ç«™å‚æ•°
...
// å»¶è¿ŸåŠ è½½å‚æ•°(æ¨¡æ¿å‰å°ç”¨æˆ·è®¾ç½®)
...
if(typeof Breakpoints != 'undefined') Breakpoints(); // çª—å£å®½åº¦æ–­ç‚¹å‡½æ•°
// jsä¸¥æ ¼æ¨¡å¼
(function(document, window, $) {
	...
	$(function() {
		Site.run();
		// ä¸­é—´å¼¹çª—éšè—æ•ˆæžœä¼˜åŒ–ï¼ˆç‚¹å‡»å¼¹çª—æ¡†å¤–ä¸Šä¸‹æ–¹éšè—å¼¹çª—ï¼‰
		...
		// å¼¹çª—é«˜åº¦è¿‡é«˜æ—¶ï¼Œå…¶ä½ç½®å–æ¶ˆåž‚ç›´å±…ä¸­
...
$.extend({
	 ...
	 * @param Number   num_start æ–‡ä»¶åŠ è½½æŽ’åºå¼€å§‹å€¼
	 * @param Number   num_end   æ–‡ä»¶åŠ è½½æŽ’åºç»“æŸå€¼
	 * @param Function fun       å›žè°ƒå‡½æ•°
	 */
	includeFile: function(file, num_start, num_end, fun, special) {
		...
		if(includeFileIndex >= num_start && includeFileIndex < num_end) {
			if(ext[0] == 'js') {
				...
				if($('link[href="' + href + '"]').length) loadFun(); // æ–‡ä»¶åŠ è½½å®Œæˆå›žè°ƒ
			}
	...
	/**
	 * include å¼‚æ­¥åŠ è½½æ–‡ä»¶é›†åˆ
	 * @param  {String}   file æ–‡ä»¶è·¯å¾„
	 * @param  {function} fun  æ–‡ä»¶åŠ è½½å®Œæˆå›žè°ƒ
	 */
...
// ajaxå¤šæ¬¡åŠ è½½ç›¸åŒæ–‡ä»¶åˆ¤æ–­ï¼Œå®šä¹‰ä¸€ä¸ªå…¨å±€scriptçš„æ ‡è®°æ•°ç»„ï¼Œç”¨æ¥æ ‡è®°æ˜¯å¦æŸä¸ªscriptå·²ç»ä¸‹è½½åˆ°æœ¬åœ°
window.scriptsArray = [];
$.cachedScript = function(url, options) {
	// å¾ªçŽ¯scriptæ ‡è®°æ•°ç»„
	for(var s in scriptsArray) {
		// å¦‚æžœæŸä¸ªæ•°ç»„å·²ç»ä¸‹è½½åˆ°äº†æœ¬åœ°
		if(scriptsArray[s] == url) {
			return { // åˆ™è¿”å›žä¸€ä¸ªå¯¹è±¡å­—é¢é‡ï¼Œå…¶ä¸­çš„doneä¹‹æ‰€ä»¥å«åšdoneæ˜¯ä¸ºäº†ä¸Žä¸‹é¢$.ajaxä¸­çš„doneç›¸å¯¹åº”
				done: function(method) {
					if(typeof method == 'function') { // å¦‚æžœä¼ å…¥å‚æ•°ä¸ºä¸€ä¸ªæ–¹æ³•
						method();
					}
				}
			};
	...
	// è¿™é‡Œæ˜¯jqueryå®˜æ–¹æä¾›ç±»ä¼¼getScriptå®žçŽ°çš„æ–¹æ³•ï¼Œä¹Ÿå°±æ˜¯è¯´getScriptå…¶å®žä¹Ÿå°±æ˜¯å¯¹ajaxæ–¹æ³•çš„ä¸€ä¸ªæ‹“å±•
	options = $.extend(options || {}, {
		dataType: "script",
		url: url,
		cache: true // å…¶å®žçŽ°åœ¨è¿™ç¼“å­˜åŠ ä¸Žä¸åŠ æ²¡å¤šå¤§åŒºåˆ«
	});
	scriptsArray.push(url); // å°†urlåœ°å€æ”¾å…¥scriptæ ‡è®°æ•°ç»„ä¸­
	return $.ajax(options);
};
...
$(function() {
			...
			var interval_lazyload_time = 0,
				interval_lazyload = setInterval(function() {
					...
					if(typeof $.fn.lazyload == 'function') {
						...
						console.log('lazyloadæ’ä»¶æ²¡æœ‰åŠ è½½ï¼');
						clearInterval(interval_lazyload);
					}
				}, 50);
		}
	...
	if($('[boxmh-mh]').length) $('[boxmh-mh]').boxMh('[boxmh-h]'); //å·¦å³åŒºå—æœ€å°é«˜åº¦è®¾ç½®
	// ä¾§æ å›¾ç‰‡åˆ—è¡¨
...
// å…¨å±€å‡½æ•°
$.fn.extend({
	// é€‰é¡¹å¡åˆ—è¡¨æ°´å¹³æ»šåŠ¨å¤„ç†ï¼ˆéœ€è°ƒç”¨swiperæ’ä»¶ï¼‰
	...
	imageloadFun: function(fun) {
		$(this).each(function() {
			if($(this).data('lazy') || $(this).data('original')) { // å›¾ç‰‡å»¶è¿ŸåŠ è½½æ—¶
				var thisimg = $(this),
	...
	/**
	 * imageSize å›¾ç‰‡é«˜åº¦é¢„è®¾åŠåˆ é™¤
	 * @param    {String} imgObj ç›®æ ‡å›¾ç‰‡ç±»
	 */
	...
	/**
	 * scrollFun çª—å£è·ç¦»è§¦å‘
	 * @param  {Number}  top            ç¦»çª—å£è§¦å‘çš„è·ç¦»ï¼Œé»˜è®¤ä¸º30
	 * @param  {Boolean} loop           æ˜¯å¦å¾ªçŽ¯è§¦å‘ï¼Œé»˜è®¤ä¸å¾ªçŽ¯è§¦å‘
	 * @param  {Boolean} skip_invisible ä¸å¯è§å…ƒç´ çš„æ˜¯å¦è§¦å‘äº‹ä»¶ï¼Œé»˜è®¤ä¸è§¦å‘
	 */
	scrollFun: function(fun, options) {
		if(typeof fun === "function") {
			...
			$(this).each(function() {
				var $self = $(this),
					fun_open = true,
					windowDistanceFun = function() { // çª—å£è·ç¦»è§¦å‘å›žè°ƒ
						if(fun_open) {
	...
	/**
	 * appearDiy æ‰‹åŠ¨appearåŠ¨ç”»ï¼ˆéœ€è°ƒç”¨appearæ’ä»¶ï¼‰
	 */
	appearDiy: function() {
	...
	/**
	 * galleryLoad ç”»å»Šï¼ˆéœ€è°ƒç”¨lightGalleryæ’ä»¶ï¼‰
	 * @param  {Array} dynamic è‡ªå®šä¹‰å›¾ç‰‡æ•°ç»„
	 */
	galleryLoad: function(dynamic) {
		if(typeof $.fn.lightGallery == 'undefined') return false;
		$("body").addClass("met-lightgallery"); //ç”»å»Šçš®è‚¤
		if(dynamic) {
			// è‡ªå®šä¹‰å›¾ç‰‡æ•°ç»„
			$(this).lightGallery({
				loop: true,
		...
		if($(this).length && $(boxmh_h).length) {
			var $self = $(this),
				...
				box_mh = function() {
					...
					if(boxmh_mh_t == boxmh_h_t) { //ä¸¤ä¸ªåŒºå—å¹¶æŽ’æ—¶
	...
	// è¡¨æ ¼å“åº”å¼æ ¼å¼åŒ–ï¼ˆéœ€è°ƒç”¨tablesawæ’ä»¶ï¼‰
	tablexys: function() {
		var $self = $(this);
		$self.each(function() {
