...
METUI_FUN['server_list_met_m1156_1']=METUI['server_list_met_m1156_1_x']={
	name: 'server_list_met_m1156_1',
	init: function(){
		if($('.swiper-header').length==0){
			METUI['server_list_met_m1156_1_x'].slide(1);
		}		
	},
	resize: function(res){
		METUI['server_list_met_m1156_1'].height($(window).height());
		if(!res) $(window).resize(function(){ METUI['server_list_met_m1156_1_x'].resize(true); });
	},
	slide: function(str){
		switch (str){
			case 1:
				if(!METUI['slide']){
					METUI['server_list_met_m1156_1']
						.css('background-image','url('+METUI['server_list_met_m1156_1'].attr('data-background')+')')
						.removeAttr('data-background');
				}
				METUI['server_list_met_m1156_1'].addClass('active');
			break;
			case 2:
				METUI['server_list_met_m1156_1'].addClass('active');
			break;
			case 3:
				METUI['server_list_met_m1156_1'].removeClass('active');
			break;
		}
	}
}
var x=new metui(METUI_FUN['server_list_met_m1156_1']);
...
METUI_FUN['show_met_m1156_1']=METUI['show_met_m1156_1_x']={
	name: 'show_met_m1156_1',
	init: function(){
		if($('.swiper-header').length==0){
			METUI['show_met_m1156_1_x'].slide(1);
		}	
	},
	resize: function(res){
		
		if(!res) $(window).resize(function(){ METUI['show_met_m1156_1_x'].resize(true); });
	},
	slide: function(str){
		switch (str){
			case 1:
				if(!METUI['slide']){
					METUI['show_met_m1156_1']
						.css('background-image','url('+METUI['show_met_m1156_1'].attr('data-background')+')')
						.removeAttr('data-background')
				}
				if(METUI['show_met_m1156_1'].find(".met-editor img").length){
					if (!METUI['show_met_m1156_1'].find(".met-editor.no-gallery").length) {
						METUI['show_met_m1156_1'].find(".met-editor").wrapInner("<div class='editorlightgallery'></div>");
						METUI['show_met_m1156_1'].find(".met-editor").each(function() {
							var img_gallery_open=1,
								this_editor=this;
							METUI['show_met_m1156_1'].find("img",this).one('click',function() {
								if(img_gallery_open){
									METUI['show_met_m1156_1'].find('img',this_editor).each(function() {
										if($(this).parent("a").length==0){
											var data_thumb=data_src = ($(this).attr("data-src")||$(this).attr("src"));
											$(this).wrap("<div class='lg-item-box' data-src='"+data_src+"' data-exthumbimage='"+data_thumb+"'></div>");
										}
									});
									METUI['show_met_m1156_1'].find('.editorlightgallery',this_editor).galleryLoad();
...
var x=new metui(METUI_FUN['show_met_m1156_1']);
